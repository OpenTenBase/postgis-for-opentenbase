...
SELECT COUNT(1) FROM (SELECT 't101', id, ST_ClusterDBSCAN(geom, eps := 0.8, minpoints := 1) OVER () from dbscan_inputs);

/* minpoints = 4, no clusters */
SELECT COUNT(1) FROM (SELECT 't102', id, ST_ClusterDBSCAN(geom, eps := 0.8, minpoints := 4) OVER () from dbscan_inputs);

/* minpoints = 3, but eps too small to form cluster on left */
SELECT COUNT(1) FROM (SELECT 't103', id, ST_ClusterDBSCAN(geom, eps := 0.6, minpoints := 3) OVER () from dbscan_inputs);
